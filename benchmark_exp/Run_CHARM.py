...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="Running CHARM")
    parser.add_argument(
        "--filename", type=str, default="001_NAB_id_1_Facility_tr_1007_1st_2014.csv"
    )
    parser.add_argument("--data_direc", type=str, default="../Datasets/TSB-AD-U/")
    parser.add_argument("--AD_Name", type=str, default="CHARM")
    ...
    output = run_CHARM(data_train, data, **CHARM_HP)

    pred = output > (np.mean(output) + 3 * np.std(output))
    evaluation_result = get_metrics(
        output, label, slidingWindow=slidingWindow, pred=pred
    )
